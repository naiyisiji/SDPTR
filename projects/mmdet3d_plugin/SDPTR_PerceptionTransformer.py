...
from projects.mmdet3d_plugin.modules.builder import build_fuser, FUSERS
from typing import List

@TRANSFORMER.register_module()
class SDPTRPerceptionTransformer(BaseModule):
    def __init__(self,
                 ...
                 modality='vision',
                 **kwargs):
        super(SDPTRPerceptionTransformer, self).__init__(**kwargs)
        if modality == 'fusion':
            self.fuser = build_fuser(fuser) #TODO
        self.use_attn_bev = encoder['type'] == 'BEVFormerEncoder'
        self.encoder = build_transformer_layer_sequence(encoder)
        self.anchor_generator = build_transformer_layer_sequence(anchor_generator)
    ...
    def forward(self,
                ...
                grid_length=[0.512, 0.512],
                sdmap_feats=None,
                anchor_ele_expand=None, 
                anchor_pts_expand=None,
                graph_label = None,
                bev_pos=None,
                reg_branches=None,
                cls_branches=None,
        ...
        query_pts, query_ele, pts_pos = self.anchor_generator(query = query,
                                                 sdmap_feats=sdmap_feats, 
                                                 anchor_ele = anchor_ele_expand,
                                                 anchor_pts = anchor_pts_expand,
                                                 bev_feats=bev_embed, 
                                                 lidar_feats=lidar_feat,
                                                 graph_label=graph_label)
        reference_points = anchor_pts_expand.view(bs, -1, 2)
        init_reference_out = reference_points

        query = query_pts.permute(1, 0, 2)
        query_pos = pts_pos.permute(1, 0, 2)
        ...
        return bev_embed, inter_states, init_reference_out, inter_references_out, query_ele
